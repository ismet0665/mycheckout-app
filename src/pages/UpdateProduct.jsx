...
const UpdateProduct = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      navigate(-1); // gelinen sayfaya geri döner.
    } catch (error) {
      console.log(error);
    }
  };
  ...
  return (
    <div className="container">
      <ProductForm
        ...
        formData={formData}
        text="Update" // başlık ve butonu dinamik yapmak için props gönderiyoruz.
      />
    </div>
  );
};
...
// ProductCard dan navigate = useNavigate(); ile edit işlemi için  const editProduct = () => {navigate("/update-product", { state: item });};  UpdateProduct a gönderdik. update butonu ile. gelen veriyi de  const { state: item } = useLocation(); hook u ile karşıladık.
